...
@section('title', $page_title ?? 'Upgrade Package')
@section('content')
<style>
    .list-unstyled li {
        margin-bottom: 20px;
...
<section class="content-header">
    ...
    <div class="content-header-right">
        ...
        <a href="{{ route('admin.company_employee.index') }}" class="btn btn-primary btn-sm">Back to Resources</a>
    </div>
</section>
            ...
            <div class="box box-default">
                ...
                <div class="box-body">
                    <ul class="list-unstyled" style="margin-bottom: 16px;">
                        <li><i class="fa fa-users" style="color: #081e37;"></i> <strong>{{ $package['employees'] }}</strong> employees included</li>
                        <li><i class="fa fa-user-tie" style="color: #081e37;"></i> <strong>{{ $package['clients'] }}</strong> clients included</li>
                        <li class="mt-2"><strong>${{ number_format($package['amount'], 2) }}</strong> one-time</li>
                    </ul>
...
{{-- Payment modal: PayPal or Card, no redirect to website --}}
<div class="modal fade" id="paymentModal" tabindex="-1" role="dialog" aria-labelledby="paymentModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header" style="border-bottom: 1px solid #f4f4f4;">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="paymentModalLabel" style="color: #081e37;">Complete upgrade — ${{ number_format($package['amount'], 2) }}</h4>
            </div>
            <div class="modal-body">
                @if ($billing_addresses->isEmpty())
                    <div class="alert alert-warning">Please <a href="{{ route('billing_address.create') }}" target="_blank">add a billing address</a> first, then refresh this page.</div>
                @else
                    <div class="form-group">
                        ...
                        <select class="form-control" id="modal_billing_address" name="billing_address_id" required>
                            @foreach ($billing_addresses as $addr)
                                <option value="{{ $addr->id }}">{{ trim(implode(', ', array_filter([$addr->first_name, $addr->last_name, $addr->street, $addr->town, $addr->country]))) ?: 'Billing address' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <p class="text-muted small" style="margin-bottom: 16px;">Total: <strong>${{ number_format($package['amount'], 2) }}</strong> (package only)</p>
                    <form action="{{ route('company.package-upgrade.paypal') }}" method="POST" style="margin-bottom: 16px;" id="paypal-form">
                        @csrf
                        <input type="hidden" name="billing_address_id" id="paypal_billing_id" value="{{ $billing_addresses->first()->id ?? '' }}">
                    ...
                    @if(config('services.authorize.api_login_id') && config('services.authorize.client_key'))
                    ...
                    <div class="card-form">
                        ...
                        <form action="{{ route('company.package-upgrade.charge') }}" method="POST" id="card-form">
                            @csrf
                            <input type="hidden" name="billing_address_id" id="card_billing_id" value="{{ $billing_addresses->first()->id ?? '' }}">
...
@if(config('services.authorize.api_login_id') && config('services.authorize.client_key'))
<script src="{{ config('services.authorize.mode') === 'live' ? 'https://js.authorize.net/v1/Accept.js' : 'https://jstest.authorize.net/v1/Accept.js' }}" charset="utf-8"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (cardForm && submitCardBtn) {
        submitCardBtn.addEventListener('click', function() {
            ...
            var authData = { clientKey: '{{ config("services.authorize.client_key") }}', apiLoginID: '{{ config("services.authorize.api_login_id") }}' };
