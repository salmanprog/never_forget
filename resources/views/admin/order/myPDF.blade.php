            ...
            <tr align="center">
				...
				<td>
					@if($order_detail->hasProduct?->image)
						<img  src="{{ public_path('admin/assets/images/product/' . $order_detail->hasProduct->image) }}" alt="Preview" style="height:70px; width:70px">
					@else
						<img src="{{ public_path('admin/assets/images/product/no-photo1.jpg') }}" alt="Preview" style="height:70px; width:70px">
					@endif
				</td>
                <td>${{number_format ($order_detail->price??'00', 2) }}</td>
                <td>
                    @if(isset($order_detail->hasProduct))
                        {{ $order_detail->hasProduct->product_type == 1 ? 'Variable Product' : 'Simple Product' }}
                    @else
                        N/A
                    @endif
                </td>  
