...
@section('title', $page_title)
    <style>
        ...
        .zoom-container {
            position: relative;
            overflow: hidden;
            cursor: url('{{ asset('public/assets/images/icons/arrow-left.svg') }}') 16 16, auto;
            width: 100%;
            height: auto;
            ...
            <div class="row">
                <div class="col-lg-6">
                    ...
                    <div class="product-slide">
                        ...
                        <div class="zoom-container">
                            <img class="zoom-img" src="{{ $firstImageUrl }}" alt="{{ $product->name }}">
                        </div>
                    </div>

                    <!-- Thumbnails -->
                    @if($mainImageUrl || count($relatedImages) > 0)
                    <div class="product-sm mt-3"> {{-- Added mt-3 for spacing --}}
                        @if($mainImageUrl)
                            <div>
                        ...
                        @foreach($relatedImages as $image)
                            <div>
                                <img src="{{ asset('public/admin/assets/images/product/' . $image) }}" alt="{{ $product->name }}">
                            </div>
                        @endforeach
                    </div>
                    @endif
                ...
                <div class="col-lg-6">
                    <div class="pd-sec-right field-wrapper">
                        <a href="" class="fs-17 heading"><span>{{ $product->hasCategory ? $product->hasCategory->title : 'N/A' }}</span></a>
                        <h2 class="heading fs-30 mb-20 mt-20">{{ $product->name }}</h2>
                        <h5 class="price heading fs-25 fw-600 mb-20">
                            @if($variations->isNotEmpty())
                                ${{ number_format($variations->min('price'), 2) }} - ${{ number_format($variations->max('price'), 2) }}
                            @else
                                ${{ number_format($product->product_price, 2) }}
                            @endif
                        </h5>
                        @if($variations->isNotEmpty())
                        {{-- <label for="" class="label-field heading fs-20 mb-10">Select</label> --}}
                        <select class="input-field form-select mb-10" id="productVariantSelect" name="variation_id">
                            <option value="">Choose an option</option>
                            @foreach($variations as $variation)
                            <option value="{{ $variation['id'] }}" 
                                    data-price="{{ $variation['price'] }}"
                                    data-img="{{ $variation['image'] ? asset('public/admin/assets/images/product/variations/'.$variation['image']) : '' }}">
                                {{ $variation['name'] }}
                            </option>
                            @endforeach
                        </select>
                        ...
                        <div class="d-flex align-items-center gap-20 mb-20">
                            <form action="{{ route('cart.store') }}" method="POST" class="d-flex align-items-center gap-20" id="addToCartForm">
                                @csrf
                                <input type="hidden" name="product_id" value="{{ $product->id }}">
                                <input type="hidden" name="product_type" value="{{ $product->product_type }}">
                                <input type="hidden" name="variation_id" id="variation_id" value="">
                                <input type="hidden" name="product_price" id="product_price" value="{{ $variations->isNotEmpty() ? 0 : $product->product_price }}">
            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="gift-card-wrapper">
                        ...
                        @endphp
                        <img src="{{ $imageUrl }}" class="w-100 mb-10" alt="{{ $related_product->name }}">
                        <h5 class="pl-20 heading fs-24 mb-10">{{ $related_product->name }}</h5>
                        <div class="mb-30 pl-20 d-flex align-items-center justify-content-between">
                            <div>
                                @if($related_product->product_type == 0)
                                    <span class="text-secondry-theme fw-700 fs-24">${{number_format($related_product->product_price, 2)}}</span>
                                @else
                            ...
                            <div>
                                ...
                                <span class="text-primary-theme-light fw-600 fs-20">{{ $related_product->rating ?? '4.8' }}</span>
                            </div>
                        </div>
                        <a href="{{ route('single-product', $related_product->slug) }}" class="pl-20 fs-20 fw-600 text-primary-theme-light">View Details
                            <span class="ms-3">
                                <img src="{{ asset('public/assets/website/images/arrow-right.png') }}" alt="">
                            </span>
                        </a>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            const defaultImageUrl = '{{ $firstImageUrl }}'; 
