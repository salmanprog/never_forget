...
<tr class="enquiry-summary-row">
    ...
    <td>
        <button type="button" class="btn btn-xs btn-default btn-toggle-enquiry-detail" data-target="quality-logo-detail-{{ $enquiry->id }}" aria-expanded="false">
            <i class="fa fa-chevron-down"></i> View details
        </button>
    </td>
</tr>
<tr class="enquiry-detail-row" id="quality-logo-detail-{{ $enquiry->id }}" style="display: none;">
        ...
        <div class="p-3">
            ...
            <table class="table table-bordered table-condensed mt-2 mb-0">
                <tbody>
                    ...
                    @endif
                    @if(!empty($enquiry->product_name))
                        <tr><th width="180">Product</th><td>{{ $enquiry->product_name }}</td></tr>
                    @endif
